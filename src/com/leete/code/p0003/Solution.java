...
/**
 * 思路：
 * 解法一：遍历所有子串，判断是不有重复的字母
 * 解法二：在[i,j)中是否有重复字母j1，，没有ｊ++，并记录当前最长 长度；有，则i++;再重复上一步
 * 解法三：在[i,j)中是否有重复字母j1，没有ｊ++，并记录当前最长 长度；有，则 i= j1在[i,j]中的index ;再重复上一步。index用HashMap来存储
 * 解法四：在[i,j)中是否有重复字母j1，没有ｊ++，并记录当前最长 长度；有，则 i= j1在[i,j]中的index ;再重复上一步。index用int[]数组来存储
 */
